...
import { Youtube, Github, Search, Loader2, AlertTriangle, ShieldCheck, ShieldAlert, Sparkles, Wand2, Trash2, Ban, Info } from 'lucide-react';
...
const Analyze = () => {
  ...
  const filteredComments = results?.items?.filter((item: any) => {
    ...
    if (activeTab === 'toxic') return ['toxic', 'spam', 'abusive'].includes(item.moderation.label);
    if (activeTab === 'safe') return item.moderation.label === 'safe';
    return true;
  }) || [];
    ...
    { 
      ...
      accessor: (c: any) => (
        <div className="max-w-md">
          <p className="font-medium text-[var(--text-primary)] line-clamp-2">{c.text}</p>
    ...
    { 
      ...
      accessor: (c: any) => (
        <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
          ['toxic', 'spam', 'abusive'].includes(c.moderation.label) ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-green-500/10 text-green-500 border-green-500/20'
        }`}>
          {c.moderation.label}
        </span>
      )
      ...
      accessor: (c: any) => (
        <div className="flex items-center gap-2">
           <GlowButton variant="secondary" className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest flex items-center gap-2">
              {['toxic', 'spam', 'abusive'].includes(c.moderation.label) ? <Trash2 size={12} /> : <ShieldCheck size={12} />}
              {['toxic', 'spam', 'abusive'].includes(c.moderation.label) ? 'Remove' : 'Approve'}
           </GlowButton>
        </div>
      )
    }
        ...
        {showResults && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <StatCard title="Toxicity Index" value={`${Math.round((results.summary.toxic_total / results.summary.total) * 100)}%`} icon={<AlertTriangle size={20} className="text-red-400" />} className="border-red-500/20" />
              ...
              <StatCard title="Safety Rating" value={`${Math.round((results.summary.safe / results.summary.total) * 100)}/100`} icon={<ShieldCheck size={20} className="text-green-400" />} className="border-green-500/20 shadow-[0_0_20px_rgba(34,197,94,0.1)]" />
            </div>
